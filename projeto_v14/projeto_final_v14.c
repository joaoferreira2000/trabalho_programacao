...
int main ()
{
    ...
    do{
        ...
        if (ask == 121 || ask == 89)
        {
            ...
            gets(ficheiro);
            FILE *fptr;
            fptr=fopen(ficheiro, "wb");
            fwrite(novo_pc, sizeof(t_pc), 25, fptr);
            fwrite(nova_intre, sizeof(t_solicitacao), 100, fptr);
            fwrite(reg_intre, sizeof(t_intrevencao), 100, fptr);
            fwrite(&numero, sizeof(cont), 1, fptr);
            fclose(fptr);
        }
...
void adc_registo(t_pc novo_pc [],t_solicitacao nova_intre[],t_intrevencao reg_intre[],cont numero)
{
